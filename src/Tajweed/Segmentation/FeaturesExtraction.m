classdef FeaturesExtraction < handle
    %UNTITLED3 Summary of this class goes here
    %   Detailed explanation goes here
    
    methods(Static=true)
        function [CC,FBE, frames] = Mfcc(signal, seg)
            [CC, FBE, frames] =  FeaturesExtraction.mfcc( signal.data, signal.fs, seg.windowTime, seg.shiftTime, ...
                seg.preemphisizeAlpha, @hamming, [seg.lowFrequency seg.highFrequency], ...
                seg.filterbanks,seg.cepstralCoefficients+1, seg.lifters);
        end
    end
    ...
    methods(Access=private, Static=true)
        ...
        function [ CC, FBE, frames ] = mfcc( speech, fs, Tw, Ts, alpha, window, R, M, N, L )
            ...
            frames = FeaturesExtraction.vec2frames( speech, Nw, Ns, 'cols', window, false );
            
            % Magnitude spectrum computation (as column vectors)
            MAG = abs( fft(frames,nfft,1) );
            
            % Triangular filterbank with uniformly spaced filters on mel scale
            H = FeaturesExtraction.trifbank( M, K, R, fs, hz2mel, mel2hz ); % size of H is M x K
